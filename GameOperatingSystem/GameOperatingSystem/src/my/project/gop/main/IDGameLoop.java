...
public class IDGameLoop extends JPanel implements Runnable {
	...
	private int tps; //ticks per second (number of cycles of game loop per second)
	...
	public void run() {
		...
		double numSecsPerTick = 1000000000D / currentFPS;
		int frames = 0;
		int ticks = 0;
		...
		while(running){
			...
			deltaTime += (now - lastTime) / numSecsPerTick;
			lastTime = now;
